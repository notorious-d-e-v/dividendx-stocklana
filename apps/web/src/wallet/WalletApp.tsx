...
export function WalletApp({ runtimeConfig = RUNTIME_CONFIG, onReset }: { runtimeConfig?: RuntimeConfig; onReset?: () => Promise<void> }) {
  ...
  return <><a className="p-skip" href="#wallet-main">Skip to content</a>
    <header className="p-header wallet-header"><a className="p-brand" href="/app/"><Mark />DividendX</a>
    ...
    <main id="wallet-main" className="p-page wallet-page">
      ...
      {tab === 'market' && <section className="wallet-market-hero"><div><p className="eyebrow">DividendX · {runtimeLabel}</p><h1>One stock.<span>Two tokens.</span></h1><p>Split a stock token into stock exposure (PT) and dividend rights (DR) for a selected year. Explore the available profiles, then connect a wallet to try it.</p></div><StockSplitHeroArt /></section>}
    ...
    </main><footer className="p-footer wallet-footer"><div><Mark /><span>DividendX{tab === 'market' ? '' : ` · ${seriesName}`}</span></div></footer>
