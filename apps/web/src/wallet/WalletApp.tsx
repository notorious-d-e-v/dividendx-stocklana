...
import { loadManifest, RuntimeRequestError, runtimePost, verifyRuntimeIdentity } from './runtime';
import {
  compatibleAccount,
  connectWallet,
...
export function WalletApp() {
  ...
  useEffect(() => {
    const refresh = () => setWallets(discoverWallets());
    refresh();
  ...
  useEffect(() => {
    ...
    return watchWallet(connected.wallet, ({ accounts, chains, features }) => {
      ...
      if (chains || features) {
        ...
        if (!isCompatibleWallet(connected.wallet)) {
      ...
      const account = compatibleAccount(sameAddress) ?? compatibleAccount(accounts);
  ...
  }, [connected?.wallet, connected?.account.address, connected?.temporary]);
  ...
  const connect = async (wallet: CompatibleWallet, temporary = false) => {
    setError('');
    try { const account = await connectWallet(wallet); setConnected({ wallet, account, temporary }); setNotice(`${wallet.name} connected for local signing.`); }
  ...
  const faucet = async () => {
    ...
    const captured = key; submitting.current = true; setBusy(true); setError(''); setNotice('Requesting bounded local test assets…');
    ...
    try {
      ...
      setNotice(`${confirmed.length} local faucet transaction${confirmed.length === 1 ? '' : 's'} confirmed.`);
      await refreshSnapshot();
    } catch (cause) {
  ...
  const runtimeLabel = verifiedManifest.kind === 'surfnet' ? 'Local SBF sandbox' : 'Local validator';
  ...
  return <><a className="p-skip" href="#wallet-main">Skip to content</a>
    ...
    <main id="wallet-main" className="p-page wallet-page">
      ...
      <section className="wallet-connect"><div><p className="eyebrow">Wallet</p><h2>{connected ? connected.wallet.name : 'Connect your wallet or create a test wallet.'}</h2>{connected ? <p><code>{connected.account.address}</code>{connected.temporary && ' · reload loses this disposable key'}</p> : <p>Use a wallet to try the local flow. The temporary wallet exists only in this browser tab.</p>}</div><div className="wallet-buttons">{connected ? <button onClick={() => { setConnected(undefined); setNotice('Wallet disconnected.'); }}>Disconnect</button> : <>{wallets.map((wallet) => <button key={wallet.name} onClick={() => void connect(wallet)}>{wallet.name}</button>)}<button data-testid="temporary-wallet" onClick={() => void connect(createTemporaryWallet(), true)}>Create temporary test wallet</button>{wallets.length === 0 && <small>No compatible installed wallet found.</small>}</>}</div></section>

      {notice && <div className="p-success" role="status">{notice}</div>}
      {error && <div className="p-error" role="alert">{error}</div>}
      {snapshotStale && <div className="p-error" role="status" data-testid="stale-balances">Balances may be out of date because the local RPC stopped responding. Refresh before continuing.</div>}

      <section className="wallet-selector"><label><span>Exact local stock token</span><select value={asset?.id ?? ''} onChange={(event) => chooseAsset(event.target.value)}>{verifiedManifest.assets.map((candidate) => <option key={candidate.id} value={candidate.id}>{candidate.company} · {candidate.symbol} · {candidate.issuerLabel}</option>)}</select></label><label><span>Annual series</span><select value={series?.year ?? ''} onChange={(event) => setYear(Number(event.target.value))}>{asset?.series.map((candidate) => <option key={candidate.year} value={candidate.year}>{candidate.year}</option>)}</select></label><div className="wallet-selector-actions"><button disabled={loadingSnapshot} onClick={() => void refreshSnapshot()}>Refresh balances</button><button disabled={busy || !holder} onClick={faucet}>Request test SOL + {asset?.symbol}</button></div></section>
      ...
      <details className="clock-controls"><summary>Network-wide test dates</summary><div className="clock-layout"><div><p className="eyebrow">Controlled annual lifecycle</p><h2>Annual lifecycle controls</h2>{verifiedManifest.clockControl ? <p>These steps change the shared local network and submit real program transactions.</p> : <p>Faithful clock control is unavailable in this runtime. Finalized claims may be inspected only when already present onchain.</p>}</div><div>{['start-year', 'record-dividends', 'end-year', 'finalize'].map((step) => <button key={step} disabled={!verifiedManifest.clockControl || busy} onClick={() => void advance(step)}>{step.replace('-', ' ')}</button>)}</div></div></details>

      {receipts.length > 0 && <section className="wallet-receipts"><p className="eyebrow">Confirmed receipts</p>{receipts.map((receipt) => <article key={`${receipt.signature}-${receipt.label}`}><div><b>{receipt.label}</b><span>Slot {receipt.slot} · {receipt.status}</span></div><code>{receipt.signature}</code></article>)}</section>}
    </main><footer className="p-footer wallet-footer"><div><Mark /><span>DividendX local transaction application · {seriesName}</span></div><a href="/rehearsal/">Historical rehearsal</a></footer>
  </>;
}
