  ...
  return <details className="demo-evidence" onToggle={(event) => onOpen(event.currentTarget.open)}>
    ...
    <div className="evidence-body">
      {state?.snapshot ? <>
        ...
        <div className="identity-list"><p><span>Stock holder</span><code>{state.snapshot.provider.address}</code></p><p><span>Dividend buyer</span><code>{state.snapshot.buyer.address}</code></p><p><span>Test USDC provenance</span><b>Circle devnet mint copied locally · synthetic local balances</b></p><p><span>Canonical Circle devnet mint</span><code>{state.snapshot.quoteAsset.canonicalMint}</code></p><p><span>Observed local quote mint</span><code>{state.snapshot.mints.quote}</code></p><p><span>DivX program</span><code>{state.snapshot.dividendXProgram}</code></p><p><span>Raydium program</span><code>{state.snapshot.raydiumProgram}</code></p><p><span>Series</span><code>{state.snapshot.series}</code></p>{state.snapshot.pool && <p><span>Local Raydium pool</span><code>{state.snapshot.pool.address}</code></p>}</div>
  ...
  return <MotionConfig reducedMotion="user"><div className="demo-shell">
    ...
    <main id="demo-main">
      <section className="demo-hero">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .45 }}><p className="demo-kicker">The DivX guided tour</p><h1>One stock.<span>Two separate tokens.</span></h1><p>Follow this guided demo to understand how DivX works.</p><button className="demo-primary hero-cta" type="button" onClick={() => { scrollTo('tour-core'); if (hostedLifecycle) void hostedLifecycle.ensureReady().catch(() => undefined); }}>Start guided tour <span aria-hidden="true">↓</span></button></motion.div>
        <motion.aside className="hero-explainer" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .45, delay: .08 }} aria-label="Tour overview"><p className="demo-kicker">The idea, in one view</p><div className="hero-stock"><span>TOKENIZED STOCK · {viewState?.asset?.company ?? 'Coca-Cola'} ({viewState?.asset ? assetLabel(viewState.asset).symbol : 'KOx'}) EXAMPLE</span><b>100</b></div><div className="hero-branch" aria-hidden="true"><i /><i /></div><div className="hero-claims"><div><span>PT</span><b>Stock exposure</b></div><div><span>DR</span><b>Dividend rights</b></div></div><h2>Learn how DivX works</h2><p>DivX separates a tokenized stock into stock exposure and dividend rights. You can put them back together or use the separate tokens in DeFi.</p></motion.aside>
      </section>
      ...
      {hostedLifecycle && <div className="guided-sandbox-status" data-testid="guided-sandbox-status" role="status">
        ...
        {(hostedLifecycle.session?.status === 'expired' || hostedLifecycle.session?.status === 'failed') && !hostedLifecycle.uncertain && <button type="button" disabled={hostedLifecycle.busy} onClick={() => { void hostedLifecycle.reset().then(() => scrollTo('tour-core')).catch(() => undefined); }}>Start a fresh guided demo</button>}
        {hostedLifecycle.error && (hostedLifecycle.uncertain || (hostedLifecycle.session?.status !== 'expired' && hostedLifecycle.session?.status !== 'failed')) && <button type="button" disabled={hostedLifecycle.checking || hostedLifecycle.busy} onClick={() => { void hostedLifecycle.check().catch(() => undefined); }}>Check sandbox status</button>}
      </div>}
