...
- [Devnet](../../../planning/evidence/hosted-browser-devnet-2026-09-18-r3.json): the bounded faucet grants exactly 10 TestKOx, the browser splits one and recombines one, and all three receipts are confirmed. [Independent status evidence](../../../planning/evidence/hosted-browser-devnet-finalized-2026-09-18.json) records all three as finalized with no error. The ending wallet display is 10 stock, 0 PT and 0 DR. Public devnet has no clock controls or independent annual redemption before real maturity and qualified finalization.
- [Accelerated wallet sandbox](../../../planning/evidence/hosted-browser-sandbox-2026-09-18-r3.json): one temporary wallet funds, splits 100, advances through start year, four dividend records, end year and finalization, then redeems PT and DR independently. Eight displayed action groups cover 30 confirmed/finalized signatures. The ending wallet has 104 stock, 0 PT and 0 DR; the selected series vault, PT supply and DR supply are all zero.
- [Guided sandbox](../../../planning/evidence/hosted-browser-guided-2026-09-18.json): all nine fixed actions complete with nine checkpoints and 36 confirmed transactions, including Raydium liquidity, Test USDC purchase, removal, recombination, synthetic annual settlement, buyer DR redemption and provider PT redemption. [Independent RPC verification](../../../planning/evidence/hosted-guided-chain-verification-2026-09-18.json) checks all 36 statuses and the exact residual state: PT supply 0, DR supply 2,876 raw, vault 111 raw, pool DR 2,876 raw and provider LP 0.

The production build, 95 root tests, 60 browser tests, 36 hosted-devnet tests, 17 broker tests and 17 gateway tests pass. The core ELF is unchanged. [Two-visitor evidence](../../../planning/evidence/hosted-browser-isolation-2026-09-18-r3.json) verifies distinct runtime, deployment-domain and account identities; visitor-A-only clock advance and reset; unchanged visitor-B clock/balances; expected stale-path 410; and no unexpected errors or visible alerts. Surfpool genesis hashes need not differ. [Natural expiry evidence](../../../planning/evidence/hosted-session-natural-expiry-2026-09-18.json) shows the provider stopped without an operator stop, the broker retained the same expired session ID and the old manifest returned 410. [Operator cleanup](../../../planning/evidence/hosted-release-acceptance-cleanup-2026-09-18.json) later removed the stopped owned acceptance VMs and reconciled their ledger records. The [release proof index](../../../planning/evidence/hosted-release-2026-09-18.json) binds the serving deployment to these records. Installed Wallet Standard extensions remain unverified.

The opt-in [real hosted-browser driver](../../../docs/hosted-browser-smoke.md) performs actual transactions without mocking broker, manifest or RPC traffic. It refuses to run without `--execute true`, an explicit HTTPS/localhost origin and a new absolute JSON output path. It never repeats an unknown mutation automatically. Follow [hosting operations](../../../docs/hosting-operations.md) for quotas, evidence retention and rollback.
