...
import {
  ...
  createInitializeScaledUiAmountConfigInstruction, createMintToCheckedInstruction, getAccount,
...
import {
  CLAIM_DECIMALS, COLLATERAL_DECIMALS, DIVIDENDX_PROGRAM_ID, FLOW, RAYDIUM_CONFIG,
  ...
  SERIES_YEAR, TEST_QUOTE_DECIMALS,
} from './constants.js';
import {
...
async function requireFreshRun(connection: Connection, signers: FlowSigners, assetPolicy: PublicKey, series: PublicKey): Promise<void> {
  const infos = await connection.getMultipleAccountsInfo([
    signers.collateralMint.publicKey, signers.testQuoteMint.publicKey, assetPolicy, series,
  ], 'confirmed');
...
async function prepareDividendX(connection: Connection, manifest: ExecutionManifest, signers: FlowSigners,
  ...
  await requireFreshRun(connection, signers, assetPolicy, series.series);
  ...
  const providerQuote = getAssociatedTokenAddressSync(signers.testQuoteMint.publicKey, signers.provider.publicKey);
  const buyerDr = getAssociatedTokenAddressSync(series.drMint, signers.buyer.publicKey);
  const buyerQuote = getAssociatedTokenAddressSync(signers.testQuoteMint.publicKey, signers.buyer.publicKey);
  ...
  await record(await buildSimulateSend(connection, 'create_provider_token_accounts', signers.admin, [
    ...
    ata(signers.admin.publicKey, providerQuote, signers.provider.publicKey, signers.testQuoteMint.publicKey),
    createMintToCheckedInstruction(signers.collateralMint.publicKey, providerCollateral, signers.admin.publicKey,
      FLOW.collateralDepositRaw, COLLATERAL_DECIMALS, [], TOKEN_2022_PROGRAM_ID),
    createMintToCheckedInstruction(signers.testQuoteMint.publicKey, providerQuote, signers.admin.publicKey,
      FLOW.seedQuoteRaw + FLOW.addQuoteRaw, TEST_QUOTE_DECIMALS),
  ], [], submitted));
  await record(await buildSimulateSend(connection, 'create_buyer_token_accounts', signers.admin, [
    ...
    ata(signers.admin.publicKey, buyerQuote, signers.buyer.publicKey, signers.testQuoteMint.publicKey),
    createMintToCheckedInstruction(signers.testQuoteMint.publicKey, buyerQuote, signers.admin.publicKey,
      FLOW.buyerQuoteRaw, TEST_QUOTE_DECIMALS),
  ], [], submitted));
...
async function validateFlowMints(connection: Connection, signers: FlowSigners, known: KnownAddresses): Promise<void> {
  const addresses = [signers.collateralMint.publicKey, known.series.ptMint, known.series.drMint, signers.testQuoteMint.publicKey];
...
async function readBalances(connection: Connection, signers: FlowSigners, known: KnownAddresses,
  ...
  const addresses = [known.providerCollateral, known.series.vault, known.providerPt, known.providerDr, known.buyerDr,
    ...
    known.series.ptMint, known.series.drMint, signers.testQuoteMint.publicKey, pool.lpMint, providerLp, pool.poolId];
  const response = await connection.getMultipleAccountsInfoAndContext(addresses, { commitment: 'confirmed' });
...
function assertSeedAndAdd(before: AssetBalances, after: AssetBalances): void {
...
export async function runAmmFlow(connection: Connection, manifest: ExecutionManifest, signers: FlowSigners,
  progress?: (value: FlowProgress) => Promise<void>): Promise<FlowReceipt> {
  ...
  const known = await prepareDividendX(connection, manifest, signers, record, submitted);
  await validateFlowMints(connection, signers, known);
  ...
  assertSeedAndAdd(checkpoints.seeded, checkpoints.added);
  await publish();
  ...
  return {
    ...
    identities: { admin: signers.admin.publicKey.toBase58(), attestor: signers.attestor.publicKey.toBase58(),
      ...
      drMint: known.series.drMint.toBase58(), testQuoteMint: signers.testQuoteMint.publicKey.toBase58(),
    ...
    limits: ['Artificial DR/test-quote ratio; test quote has no value or redemption right.',
