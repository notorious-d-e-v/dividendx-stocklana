...
The runtime requires the root workspace dependencies, the built transaction SDK, the built AMM integration package with its state export, and the accepted DivX ELF at `target/deploy/dividendx.so`. Startup verifies the DivX ELF and captured Raydium ELF hashes before launching Surfpool.
