...
Build **one dividend layer for tokenized stocks on Solana**, with xStocks, Backpack and Ondo in the initial integration scope. Audit other Solana products for eligibility from the outset. Defer other-chain custody and bridging. Ondo's native Solana tokens belong in this phase even though its other deployments do not.
...
**Token compatibility is conditional.** A mint's presence on Solana does not establish that a vault can hold it, identify its dividend, or deliver its proceeds. The research found restricted registered shares, tokens without dividend rights, and discontinued products. Treat these explicitly rather than claiming every stock token works automatically. See [Solana issuer synthesis](solana-issuer-synthesis.md) and `research/` for dated evidence.
...
Store eligibility as separate dimensions: official mint recognized; mint observed; dividend rights documented; custody and claim transfer policy accepted; event source usable; deposit/redemption tested; event allocation tested; live series enabled. Discovery or passing a fixture must never set the final dimension implicitly. An unsupported asset can appear in the research directory with a plain explanation, but cannot accept a deposit.
...
Superstate-style registered shares require a separately approved custody/holder/claim-transfer design. Permissioned underlying must not be made to appear permissionless by issuing freely transferable claims around its restrictions. No claim of approval or legal compatibility follows from a successful token instruction.

Per-series collateral remains isolated. Shared UI, code and settlement currency do not make different issuer claims fungible or guarantee shared liquidity. A seeded two-wallet demo sale is sufficient for the hackathon; pooled liquidity, routing and issuer substitution are separate product work.
...
A live series additionally requires a future event, tested permitted custody/transfer behavior, finality/correction rules and an operational signer. No production readiness or mainnet transaction has been established. Other networks, an extra wrapper token, AMM integration, leverage and automatic USDC conversion remain later work.
