...
18 September hosting direction: deliver real-calendar public devnet and an online accelerated sandbox with isolated visitor sessions. No shortened devnet term or time override. The [hosting foundation](hosting-foundation-review.md) now verifies both complete runtime flows on native Vercel Linux and creates the persistent 2027 devnet registry. The [hosting plan](hosting-plan.md) retains visitor sessions, durable faucet limits and public release checks as next work. Public app/hosting work precedes the user's Ondo/Backpack outreach; qualified issuer settlement remains a separate gate.
...
| Public app and hosting — current priority | Linux/provider execution verified; persistent real-calendar devnet registry and app configuration implemented. Visitor sessions, durable funding limits and release verification remain | High |
