...
18 September hosting direction: deliver real-calendar public devnet and an online accelerated sandbox with isolated visitor sessions. No shortened devnet term or time override. The [hosting plan](hosting-plan.md) recommends a Vercel-first feasibility probe, with Cloudflare Containers as fallback. Public app/hosting work precedes the user's Ondo/Backpack outreach; qualified issuer settlement remains a separate gate.
...
| Public app and hosting — current priority | Real-calendar devnet wallet/AMM flows plus isolated online accelerated sandbox; Linux/provider probe and public browser verification required | High |
