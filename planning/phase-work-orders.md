...
Read the [prior-art review and handoff gates](research/prior-art-review.md). Astra must resolve them in the program contract first. The program handles a whole-event bearer DR, not streamed yield or per-wallet reward history. A future series does not know its final M1 at deposit. Include authenticated zero/cancellation separately from missing evidence; explicit zero-output closure; external holder burns without changing survivors' entitlements; retained donation/abandoned reserves with no sweep; and operation-specific pause/exit rules. No protocol fee, rewards token or additional wrapper in this version. Review licenses before reusing third-party source. Preserve the current fixture SDK as a versioned rehearsal contract.
...
Add the prior-art acceptance cases: sell/transfer DR both before and after settlement with no detached seller entitlement; positive versus authenticated-zero versus missing outcomes; direct burns before/after settlement; later corporate actions not reallocating frozen pools; explicit tiny-claim closure; multiple depositors; failed custody leaving claims unchanged; and valid remaining claims redeeming even when a series cannot become globally empty. Controlled synthetic exceptions are not claims about historical issuer events.
