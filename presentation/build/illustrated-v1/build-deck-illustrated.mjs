...
const TMP_DIR = path.join(workspaceDir, "build", "illustrated-v1");
const ASSET_DIR = path.join(workspaceDir, "assets", "illustrated-v1");
const FINAL_PPTX = path.join(workspaceDir, "output", "DividendX-illustrated-v1.pptx");
...
// 7 — Fractional ownership and auditability
{
 ...
 const illustration=await fs.readFile(path.join(ASSET_DIR,"claim-composability.png"));
 s.images.add({blob:illustration,contentType:"image/png",alt:"Editorial illustration of one dividend claim divided into smaller pieces and used by connected Solana applications",fit:"contain",position:{left:54,top:188,width:355,height:355}});
 label(s,"Fractional ownership",452,198,330,C.amberInk); text(s,"Buy or sell part of a dividend claim.",452,232,330,68,27,C.ink,true,{lineSpacing:1.05});
 const whole=box(s,452,342,324,46,C.paleAmber,C.amber,10,"Dividend claim"); text(s,"DIVIDEND CLAIM",470,355,288,20,14,C.amberInk,true,{align:"center"});
 const partA=box(s,452,425,140,62,C.surface,C.amber,10,"Holder part"); text(s,"holder keeps",466,444,112,22,15,C.amberInk,true,{align:"center"});
 const partB=box(s,614,425,162,62,C.paleAmber,C.amber,10,"Buyer part"); text(s,"buyer takes part",628,444,134,22,15,C.amberInk,true,{align:"center"});
 arrow(s,whole,partA,C.amber,"bottom","top"); arrow(s,whole,partB,C.amber,"bottom","top");
 rule(s,808,190,1,C.line,356);
 label(s,"Auditability",844,198,340,C.blueInk); text(s,"Inspect the onchain record.",844,234,350,64,28,C.ink,true,{lineSpacing:1.05});
 const auditRows=[["01","Vault balance"],["02","Claims outstanding"],["03","Redemption history"]];
 auditRows.forEach((r,i)=>{const y=332+i*72; text(s,r[0],844,y,40,24,14,C.muted,true); text(s,r[1],900,y-2,280,28,20,i===0?C.blue:C.ink,true); rule(s,844,y+42,340,C.line,1);});
 text(s,"A common claim interface for other Solana applications.",80,606,1080,46,25,C.ink,true);
 notes(s,"Dividend claims are divisible, so a holder can sell part of an entitlement and a buyer can take a smaller position. Onchain records let users inspect the vault balance, claims outstanding and redemption history. Other Solana applications can integrate the same claim interface.",["Onchain auditability covers held collateral and program records. It does not independently prove issuer reserves, correct issuer data, correct program code, or audit certification. Claim transferability remains subject to each eligible series' rules. Corporate-action handling remains technical Q&A: the verified KOx dividend is accepted, while the exact HONx reverse split and later spinoff are unsupported dividend events."],["Solana tokens: https://solana.com/docs/tokens","Solana accounts: https://solana.com/docs/core/accounts","Solana transactions: https://solana.com/docs/core/transactions","Token supply RPC: https://solana.com/docs/rpc/http/gettokensupply","KOx history: https://api.xstocks.fi/api/v2/public/corporate-actions/history?page=1&pageSize=100&symbol=KOx&sortBy=createdTimeUtc&sortOrder=asc","HONx history: https://api.xstocks.fi/api/v2/public/corporate-actions/history?page=1&pageSize=100&symbol=HONx&sortBy=createdTimeUtc&sortOrder=asc"]);
}
...
const candidatePath=path.join(stagingDir,"DividendX-illustrated-v1-candidate.pptx");
...
const result=await finalizePresentation({
 ...
 receiptPath:path.join(stagingDir,"DividendX-illustrated-v1.validation.json")
});
