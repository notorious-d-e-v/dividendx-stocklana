...
const TMP_DIR = path.join(workspaceDir, "build", "illustrated-v2");
...
const FINAL_PPTX = path.join(workspaceDir, "output", "DividendX-illustrated-v2.pptx");
...
const narrationMarkdown = await fs.readFile(path.join(workspaceDir, "narration.md"), "utf8");
const canonicalNarration = [];
const narrationLines = narrationMarkdown.split(/\r?\n/);
for (let i=0;i<narrationLines.length;i++) {
  const match=narrationLines[i].match(/^\*\*(\d+)\s+—/);
  if (!match) continue;
  let j=i+1;
  while (j<narrationLines.length && !narrationLines[j].trim()) j++;
  const paragraph=[];
  while (j<narrationLines.length && narrationLines[j].trim() && !narrationLines[j].startsWith("**")) paragraph.push(narrationLines[j++].trim());
  canonicalNarration[Number(match[1])-1]=paragraph.join(" ");
}
if (canonicalNarration.length!==9 || canonicalNarration.some(v=>!v)) throw new Error("Expected nine narration sections in presentation/narration.md");
...
// 1 — Cover
{
 ...
 notes(s,"DividendX lets a stock holder keep the stock exposure and sell the dividend rights. We are building a permissionless dividend market for selected stock tokens from xStocks, Backpack and Ondo on Solana.",["The protocol is not launched. Current software is an approved interface and working historical calculator. Selected integration scope does not mean every token from these issuers is eligible or already integrated."],[]);
}
...
// 2 — Two parties
{
 ...
 notes(s,"Traditional finance already has both sides: banks sell dividend exposure to manage risk from structured products; asset managers such as Survista buy discounted dividend futures. DividendX brings that choice to tokenized-stock holders: keep the stock exposure and sell the dividend rights.",["The institutions shown are participants in traditional dividend markets, not DividendX users. Bank direction varies by book and tenor; hedge funds can buy or sell. Traditional dividend futures and swaps are generally cash-settled. DividendX allocates collateral in the deposited stock token, and proceeds require a willing buyer at an agreed price."],["Eurex 2025 whitepaper, pp. 31–32: https://www.eurex.com/resource/blob/4418754/f90fd622a278beb08757729864e93925/data/whitepaper-derivatives-forum-frankfurt-2025.pdf","CME, Trading Dividend Uncertainty, 2024: https://www.cmegroup.com/articles/2024/trading-dividend-uncertainty.html","CME dividend-futures primer, 2024: https://www.cmegroup.com/articles/2024/equity-index-dividend-futures-a-primer.html"]);
}

// 3 — Mechanism
{
 const s=p.slides.add(); header(s,3,"One deposit, two claims");
 const illustration=await fs.readFile(path.join(ASSET_DIR,"stock-dividend-coupon.png"));
 s.images.add({blob:illustration,contentType:"image/png",alt:"Editorial illustration of a stock certificate separating into stock exposure and one dividend coupon",fit:"contain",position:{left:62,top:194,width:355,height:340}});
 const asset=box(s,454,294,150,88,C.ink,C.ink,14,"Stock token"); text(s,"Stock token",472,314,114,48,22,"#FFFFFF",true,{align:"center",valign:"middle"});
 const vault=box(s,648,278,168,120,C.surface,C.line,16,"Vault"); label(s,"Locked",672,296,120); text(s,"VAULT",672,329,120,42,27,C.ink,true,{align:"center"});
 const pt=box(s,880,190,305,135,C.paleBlue,C.blue,14,"Stock exposure PT"); label(s,"PT",906,211,70,C.blueInk); text(s,"Stock exposure",906,243,250,32,24,C.blueInk,true); text(s,"Stock exposure after dividend allocation",906,281,246,31,14,C.muted,false,{lineSpacing:1.08});
 const dr=box(s,880,369,305,135,C.paleAmber,C.amber,14,"Dividend rights DR"); label(s,"DR",906,390,70,C.amberInk); text(s,"Dividend rights",906,422,250,32,24,C.amberInk,true); text(s,"This event’s dividend share",906,460,246,31,14,C.muted,false,{lineSpacing:1.08});
 arrow(s,asset,vault,C.ink); arrow(s,vault,pt,C.blue); arrow(s,vault,dr,C.amber);
 text(s,"Each pair of claims has its own locked collateral.",78,571,770,38,23,C.ink,true);
 text(s,"Redeems in the deposited stock token. Dollar value can change.",78,621,810,30,17,C.muted,false);
 label(s,"Schematic, not a 50/50 allocation",924,635,280,C.muted);
 notes(s,"One deposit enters a locked vault. The program issues two paired claims: PT for the stock exposure and DR for the dividend-derived share. Each pair is backed by its own stock-token collateral and redeems in that token. Its dollar value can change.",["This describes product mechanics, not deployed execution. Each vault isolates one issuer, mint and event. The supported Scaled UI reinvestment model shares raw-unit allocation rules across qualified xStocks, Backpack and Ondo tokens. Reinvested-token settlement stays in the deposited token. No third circulating wrapper exists. Deposit must precede the supported event, and claim receipt units differ from raw redemption allocations."],["xStocks multipliers: https://docs.xstocks.fi/developers/multipliers","Backpack MU: https://learn.backpack.exchange/blog/tokenized-micron-mu","Ondo corporate actions: https://docs.ondo.finance/ondo-stocks/corporate-actions"]);
}
...
// 5 — Market context
{
 ...
 text(s,"Starting market",80,493,230,30,15,C.muted,true); text(s,"Selected dividend stocks on Solana",312,484,760,46,30,C.ink,true);
 text(s,"Initial scope: xStocks, Backpack and Ondo",312,538,760,32,18,C.blueInk,true);
 text(s,"RWA.xyz includes stocks and ETFs represented natively or synthetically.",80,625,1090,30,15,C.muted,false);
 notes(s,"Global dividends totaled 1.75 trillion dollars in 2024. RWA.xyz reported 2.92 billion dollars of tokenized-stock value on September 16, 2026. Our starting package includes familiar dividend payers such as Apple, Microsoft and Coca-Cola, represented by stock tokens on Solana.",["The RWA.xyz total is global and includes stocks, ETFs, native instruments, and synthetic representations. Eligible Solana dividend collateral is only a subset. Initial scope names issuer families under review, not a supported-token inventory or partnership announcement. The two measures are not additive TAM and do not support a growth estimate. The selected package contains 15 candidate mints across six companies: all six from xStocks and Ondo, and Micron, Nike and IBM from Backpack; this is not 15 completed integrations."],["Janus Henderson, published 2025: https://www.janushenderson.com/en-dk/advisor/press-releases/global-dividends-jumped-to-a-record-1-75-trillion-in-2024/","RWA.xyz Stocks dashboard, observed and reconfirmed 16 September 2026: https://app.rwa.xyz/stocks"]);
}

// 6 — One workflow across issuers
{
 const s=p.slides.add(); header(s,6,"One workflow across issuers");
 const rows=[
  ["01","Multiple issuers","xStocks, Backpack and Ondo feed one vault design.",C.blue],
  ["02","Fast settlement","Payment and the dividend claim move together.",C.ink],
  ["03","Separate ownership","Transfer and redeem each claim under common rules.",C.amber]
 ];
 ...
 text(s,"One shared flow. Separate collateral for each issuer.",158,560,960,40,24,C.ink,true);
 notes(s,"Solana lets stock tokens from different issuers use the same vault design. Each issuer's dividend data feeds common allocation rules. One transaction deposits collateral and creates both claims. Holders use the same deposit, trade and redemption flow, while each issuer's collateral stays separate.",["Fast settlement describes the designed secondary trade: payment and the dividend claim move atomically in one transaction or the trade reverts. It does not claim deployed execution, an immediate future dividend payout, or completed issuer integration. All three observed families use Token-2022 Scaled UI Amount, so qualified reinvested-dividend accounting can share one engine. Each issuer still needs its own event reader and custody checks. Exact mint, decimals, event source, extensions and issuer controls stay pinned per series. The engine never pools collateral, makes claims across issuers fungible, or guarantees liquidity. Permissioned holder or approved-vault products, no-dividend products and fee-bearing products are outside scope, not an upcoming integration track. Permissionless means no DividendX holder allowlist or issuer-specific approved-vault onboarding for admitted assets; underlying issuer controls and product restrictions remain. Ondo documents secondary transferability outside the United States subject to restrictions; issuer KYC for primary issuance or redemption is separate. Exact mint checks, event evidence and execution tests are required before a series opens. xStocks has public event history. Backpack MU has a fully bracketed sourced onchain reconstruction around its DividendDistribute transaction and later ordinary mint/redeem multiplier recomputations; an official event ledger and finality semantics remain unverified. Today's multiplier must not be used as an event factor. Ondo's keyed multiplier history still needs classified event joins, and no Ondo event is source-complete."],["Solana transactions: https://solana.com/docs/core/transactions","Solana institutional payments: https://solana.com/solutions/institutional-payments","xStocks multipliers: https://docs.xstocks.fi/developers/multipliers","xStocks corporate actions: https://docs.xstocks.fi/apis/openapi/corporate-actions","Backpack MU: https://learn.backpack.exchange/blog/tokenized-micron-mu","Backpack MU finalized DividendDistribute transaction: https://explorer.solana.com/tx/39vTkahE7rUnFypAkKpaHUxwMyuC1nG4nJ12GEm3y9V1Kqep6ijvcX4isqsNXB63s3vsGNDjfi7uFFgH32Te59R","Local Backpack audit: planning/evidence/backpack-scope-mu-raw-transactions-2026-09-16.json","Ondo corporate actions: https://docs.ondo.finance/ondo-stocks/corporate-actions","Ondo multiplier history API: https://docs.ondo.finance/api-reference/assets/get-shares-multiplier-history-for-an-asset","Ondo transferability: https://docs.ondo.finance/ondo-stocks/transferability","Ondo Solana repository: https://github.com/ondoprotocol/global-markets-solana"]);
}

// 7 — Both claims: composability, fractional ownership and auditability
{
 const s=p.slides.add(); header(s,7,"Both sides are composable");
 const illustration=await fs.readFile(COMPOSABILITY_ASSET);
 s.images.add({blob:illustration,contentType:"image/png",alt:"Blue stock-exposure certificate and amber dividend coupon each linked to a wallet and a trading app",fit:"contain",position:{left:44,top:173,width:548,height:411}});
 text(s,"Stock exposure (PT)",76,592,230,27,17,C.blue,true);
 text(s,"Dividend rights (DR)",348,592,230,27,17,C.amber,true,{align:"right"});
 text(s,"Use either claim in compatible wallets and trading apps.",76,628,502,25,15,C.muted,false,{align:"center"});

 rule(s,624,180,1,C.line,472);
 label(s,"Fractional ownership",668,186,480,C.muted);
 text(s,"Buy or sell part of either claim.",668,217,500,72,30,C.ink,true,{lineSpacing:1.02});
 text(s,"Stock exposure (PT)",668,308,210,24,16,C.blue,true);
 const ptHeld=box(s,884,305,174,31,C.paleBlue,C.blue,7,"PT fraction held");
 const ptTraded=box(s,1061,305,106,31,C.surface,C.blue,7,"PT fraction transferred");
 text(s,"keep",898,312,146,18,12,C.blueInk,true,{align:"center"});
 text(s,"sell",1074,312,80,18,12,C.blueInk,true,{align:"center"});
 text(s,"Dividend rights (DR)",668,359,210,24,16,C.amber,true);
 const drHeld=box(s,884,356,110,31,C.surface,C.amber,7,"DR fraction held");
 const drTraded=box(s,997,356,170,31,C.paleAmber,C.amber,7,"DR fraction transferred");
 text(s,"keep",897,363,84,18,12,C.amberInk,true,{align:"center"});
 text(s,"sell",1011,363,142,18,12,C.amberInk,true,{align:"center"});
 text(s,"Choose how much to keep or sell.",668,401,499,22,13,C.muted,false);

 rule(s,668,442,499,C.line,1);
 label(s,"Auditability",668,463,340,C.blueInk);
 text(s,"Inspect the onchain record.",668,492,499,38,25,C.ink,true);
 const auditRows=[["01","Vault balance"],["02","Claims outstanding"],["03","Redemption history"]];
 auditRows.forEach((r,i)=>{const y=543+i*36; text(s,r[0],668,y,38,22,13,C.muted,true); text(s,r[1],716,y-1,451,24,18,i===0?C.blue:C.ink,true); if(i<2) rule(s,668,y+28,499,C.line,1);});
 notes(s,"Dividend claims are divisible, so a holder can sell part of an entitlement and a buyer can take a smaller position. Onchain records let users inspect the vault balance, claims outstanding and redemption history. Other Solana applications can integrate the same claim interface.",["PT and DR may be used by compatible Solana applications subject to each eligible series' rules. Backing stays in the series vault while either claim moves or connects to an app. The illustration shows conceptual compatibility, not deployed integrations. Onchain auditability covers held collateral and program records. It does not independently prove issuer reserves, correct issuer data, correct program code, or audit certification. Corporate-action handling remains technical Q&A: the verified KOx dividend is accepted, while the exact HONx reverse split and later spinoff are unsupported dividend events."],["Approved local illustration and provenance: design/illustrations/assets/stock-and-dividend-composability-v1.png; design/illustrations/assets/manifest.json; design/illustrations/prompts/stock-and-dividend-composability-v1.md","Solana tokens: https://solana.com/docs/tokens","Solana accounts: https://solana.com/docs/core/accounts","Solana transactions: https://solana.com/docs/core/transactions","Token supply RPC: https://solana.com/docs/rpc/http/gettokensupply","KOx history: https://api.xstocks.fi/api/v2/public/corporate-actions/history?page=1&pageSize=100&symbol=KOx&sortBy=createdTimeUtc&sortOrder=asc","HONx history: https://api.xstocks.fi/api/v2/public/corporate-actions/history?page=1&pageSize=100&symbol=HONx&sortBy=createdTimeUtc&sortOrder=asc"]);
}
...
// 9 — Solana issuer scope and roadmap
{
 ...
 label(s,"Build scope and roadmap",80,382,260,C.muted); rule(s,80,414,1090,C.line,1);
 const roadmap=[["01","Initial Solana scope","xStocks · Backpack · Ondo"],["02","Selected stocks","Coca-Cola · Apple · Microsoft · Micron · Nike · IBM"],["03","Other networks later","Coinbase on Base · Robinhood Chain"]];
 ...
 notes(s,"The hackathon target is one complete flow across supported Solana stock tokens: deposit, split, sell the dividend, redeem each side separately. We start with selected stocks from xStocks, Backpack and Ondo, proving one dividend event per issuer before expanding within the package. Other networks come later, with Solana as our home base for dividend liquidity.",["This is build scope and roadmap, not a claim of completed integration or partnership. The package contains 15 candidate mints across six companies: xStocks and Ondo each include Coca-Cola, Apple, Microsoft, Micron, Nike and IBM; Backpack includes Micron, Nike and IBM only. The six companies are not all available from all three issuers. KOx has a complete source-verified replay fixture; Backpack MU has a fully bracketed sourced onchain reconstruction; Ondo event binding still needs authenticated source data. One event per issuer is the first execution target. All vault execution remains planned. Excluded restricted, no-dividend, discontinued and fee-bearing families are outside scope. Coinbase collateral is on Base. The roadmap refers to Robinhood's new wallet-held Stock Tokens on Robinhood Chain. Cross-chain custody, settlement and messaging remain future design work."],["Selected asset package: planning/research/initial-asset-package.md","xStocks: https://xstocks.fi/","xStocks docs: https://docs.xstocks.fi/docs","Backpack assets registry: https://api.backpack.exchange/api/v1/assets","Backpack MU: https://learn.backpack.exchange/blog/tokenized-micron-mu","Ondo corporate actions: https://docs.ondo.finance/ondo-stocks/corporate-actions","Ondo Solana registry: https://github.com/ondoprotocol/gm-solana-simulator/blob/0688add3c64aadc7006712989e9ec0592b5b10f8/constants.rs","Coinbase Tokenized Stocks: https://www.coinbase.com/tokenize","Robinhood Stock Tokens: https://robinhood.com/rhj/stocktokens/","Robinhood Chain: https://robinhood.com/us/en/support/articles/robinhood-chain-testnet/"]);
}

const stagingDir=path.join(TMP_DIR,"finalizer");
await fs.mkdir(stagingDir,{recursive:true});
const candidatePath=path.join(stagingDir,"DividendX-illustrated-v2-candidate.pptx");
...
const result=await finalizePresentation({
 ...
 receiptPath:path.join(stagingDir,"DividendX-illustrated-v2.validation.json")
});
