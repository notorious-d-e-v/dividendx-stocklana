...
const FINAL_PPTX = path.join(workspaceDir, "output", "DividendX-phase-two-v4.pptx");
...
// 1 — Cover
{
 ...
 notes(s,"DividendX lets a stock holder keep the stock exposure and sell the dividend rights. We are building a shared dividend market for tokenized stocks on Solana, with xStocks, Backpack and Ondo in the initial integration scope.",["The protocol is not launched. Current software is an approved interface and working historical calculator. Initial integration scope does not mean every token from these issuers is eligible or already integrated."],[]);
}
...
// 5 — Market context
{
 ...
 text(s,"Starting market",80,493,230,30,15,C.muted,true); text(s,"Dividend-paying stock tokens on Solana",312,484,760,46,30,C.ink,true);
 ...
 notes(s,"Global dividends totaled 1.75 trillion dollars in 2024. RWA.xyz reported 2.92 billion dollars of tokenized-stock value on September 16, 2026. Our starting market is dividend-paying stock tokens on Solana, across issuers.",["The RWA.xyz total is global and includes stocks, ETFs, native instruments, and synthetic representations. Eligible Solana dividend collateral is only a subset. Initial scope names issuer families under review, not a supported-token inventory or partnership announcement. The two measures are not additive TAM and do not support a growth estimate."],["Janus Henderson, published 2025: https://www.janushenderson.com/en-dk/advisor/press-releases/global-dividends-jumped-to-a-record-1-75-trillion-in-2024/","RWA.xyz Stocks dashboard, observed and reconfirmed 16 September 2026: https://app.rwa.xyz/stocks"]);
}
...
// 6 — One workflow across issuers
{
 ...
 text(s,"Each stock needs verified dividend data and transfer compatibility.",158,560,960,40,24,C.ink,true);
 notes(s,"Solana lets assets from different issuers use the same vault design. Each issuer's dividend data feeds common allocation rules. One transaction deposits collateral and creates both claims. We enable each stock after verifying its dividend data and transfer requirements, while keeping each issuer's collateral separate.",["All three observed families use Token-2022 Scaled UI Amount, so qualified reinvested-dividend accounting can share one engine. Each issuer still needs its own event reader and custody checks. Exact mint, decimals, event source, extensions and issuer controls stay pinned per series. The engine never pools collateral, makes claims across issuers fungible, or guarantees liquidity. Permissioned shares, tokens without dividend rights, transfer-fee products and missing event data are not automatically enabled. xStocks has public event history. Backpack's MU history includes an observed DividendDistribute transaction and later multiplier recomputations during ordinary mint/redeem activity; its official event feed and finality semantics remain unverified, and onchain reconstruction is being assessed. Today's multiplier must not be used as an event factor. Ondo's keyed multiplier history still needs classified event joins."],["xStocks multipliers: https://docs.xstocks.fi/developers/multipliers","xStocks corporate actions: https://docs.xstocks.fi/apis/openapi/corporate-actions","Backpack MU: https://learn.backpack.exchange/blog/tokenized-micron-mu","Backpack MU finalized DividendDistribute transaction: https://explorer.solana.com/tx/39vTkahE7rUnFypAkKpaHUxwMyuC1nG4nJ12GEm3y9V1Kqep6ijvcX4isqsNXB63s3vsGNDjfi7uFFgH32Te59R","Local Backpack audit: planning/evidence/backpack-scope-mu-raw-transactions-2026-09-16.json","Ondo corporate actions: https://docs.ondo.finance/ondo-stocks/corporate-actions","Ondo multiplier history API: https://docs.ondo.finance/api-reference/assets/get-shares-multiplier-history-for-an-asset","Ondo Solana repository: https://github.com/ondoprotocol/global-markets-solana","Galaxy registered shares: https://investor.galaxy.com/ir-resources/tokenized-glxy-shares"]);
}
...
// 9 — Solana issuer scope and roadmap
{
 ...
 const roadmap=[["01","Initial Solana scope","xStocks · Backpack · Ondo"],["02","More eligible Solana stocks","Additional assets after dividend and custody checks"],["03","Other networks later","Coinbase on Base · Robinhood Chain"]];
 ...
 notes(s,"The hackathon target is one complete flow across supported Solana stock tokens: deposit, split, sell the dividend, redeem each side separately. We build for xStocks, Backpack and Ondo now. Other networks come later, including Coinbase on Base and Robinhood Chain, with Solana as our home base for dividend liquidity.",["This is build scope and roadmap, not a claim of completed integration or partnership. Current observation covers real issuer/mint profiles. KOx has a complete source-verified replay fixture; vault execution remains planned. Each additional asset requires dividend rights, usable event data, custody compatibility and transfer tests. Restricted, no-dividend, discontinued or fee-bearing products may remain ineligible. Coinbase collateral is on Base. The roadmap refers to Robinhood's new wallet-held Stock Tokens on Robinhood Chain, not legacy Classic tokens. Cross-chain custody, settlement and messaging remain future design work."],["xStocks: https://xstocks.fi/","xStocks docs: https://docs.xstocks.fi/docs","Backpack MU: https://learn.backpack.exchange/blog/tokenized-micron-mu","Ondo corporate actions: https://docs.ondo.finance/ondo-stocks/corporate-actions","Coinbase Tokenized Stocks: https://www.coinbase.com/tokenize","Robinhood Stock Tokens: https://robinhood.com/rhj/stocktokens/","Robinhood Chain: https://robinhood.com/us/en/support/articles/robinhood-chain-testnet/"]);
}
...
const result=await finalizePresentation({
 ...
 receiptPath:path.join(stagingDir,"DividendX-phase-two-v4.validation.json")
});
