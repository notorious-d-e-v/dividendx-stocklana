...
Our Stocklana submission includes a working Solana program, a [public devnet app](https://divx.payai.network/app/) and an [accelerated guided sandbox](https://divx.payai.network/demos/). The full tour completes 40 confirmed transactions using synthetic assets and dividend events. You can inspect the deployed [DivX program on Solscan Devnet](https://solscan.io/account/2EpQ2iKz921Ce2rremdnFrqG15DQq3Y2rSXbgixceDQE?cluster=devnet) and the [source code](https://github.com/notorious-d-e-v/divx). The sandbox's private-chain transactions are separate from public devnet.
